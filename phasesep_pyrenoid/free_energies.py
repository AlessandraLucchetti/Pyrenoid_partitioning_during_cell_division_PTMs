...
class FloryHugginsNComponentsExternalPotential(FreeEnergyNComponentsBase):
    ...
    def __init__(
        ...
        simT: float = 0,
        external_potential: Optional[Callable[[float, float], np.ndarray]] = None,
        x_grid_matrix: Optional[Callable[[float, float], np.ndarray]] = None,
        y_grid_matrix: Optional[Callable[[float, float], np.ndarray]] = None,
        *,
        ...
        **kwargs,
        ...
        self._external_potential = external_potential
        self._x_grid_matrix = x_grid_matrix
        self._y_grid_matrix = y_grid_matrix
    ...
    ) -> FloryHugginsNComponentsExternalPotential:
    ...
    def free_energy(self, phi: NumberOrArray, t: float = 0) -> NumberOrArray:
        ...
        return (  # type: ignore
            ...
            + np.einsum("i,i...->...", self._external_potential[:-1], phi)  ################ New term t/self.simT * self._external_potential[:-1], phi
            + self._external_potential[-1] * phi_last  ################## Contribution of the last component
    ...
    def chemical_potential(
        ...
        *,
        ...
        for i in range(self.dim):
            ...
            out[i] +=  self._external_potential[i] ###################### # Add external potential contribution t/self.simT *
    ...
    def make_chemical_potential(
        ...
        if backend == "numpy":
            ...
            external_potential_arr = self._external_potential
            grid_arr_x = self._x_grid_matrix
            grid_arr_y = self._y_grid_matrix
            simT = self.simT
            @jit
            def mu_local(arr: np.ndarray, t: float, out: np.ndarray) -> np.ndarray:
                ...
                # X = grid_arr_x
                # Y = grid_arr_y
                # func =  a * np.exp(-((X-xc1-v1*t0)**2+(Y-yc)**2)/(2*c1**2)) + a * np.exp(-((X-xc2-v2*t0)**2+(Y-yc)**2)/(2*c2**2)) #t/simT * 2 * a * np.exp(-(X-xc)**2/(2*c**2))
                # external_potential_arr = np.stack((func, func, func))
                for index in nb.prange(arr[0].size):
                    ...
                    if phi_last <= 0:
                        ...
                        for i in range(dim):
                            ...
                            mu += internal_energies_reduced[i] - entropy_last + external_potential_arr[i].flat[index] - external_potential_arr[-1].flat[index]
                            out[i].flat[index] = mu
                return out
