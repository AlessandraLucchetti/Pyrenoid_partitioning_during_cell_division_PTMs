            ...
            for m in range(num_comp):  # iterate over all components
                if diff_arr[n, m] != 0:
                    for i in range(2):  # iterate over all dimension
                        # use diffusivities directly
                        out[n, i, ...] += diff_arr[n, m] * mu_grads[m, i, ...] #+ 0.0001 * phi[m]                 ############## ADD HYDRODYNAMIC FLOWS HERE I THINK (+V * PHI)
...
def _make_mc_flux_to_staggered_scaled_diagonal_numba_2d(
    ...
    parallel = dim_x * dim_y >= config["numba.multithreading_threshold"]
    @jit(parallel=parallel)
    def calc_flux(phi: np.ndarray, mu: np.ndarray, velocity_field: np.ndarray, out: np.ndarray) -> np.ndarray:
        """calculate fluxes driven by gradients in `mu`"""
        # determine the fluxes from the thermodynamic force on the flux
        for n in range(num_comp):  # iterate over all components
...
def _make_mc_flux_to_staggered_advection_scaled_diagonal_numba_2d(
    grid: CartesianGrid,
    num_comp: int,
    diffusivity: Optional[np.ndarray] = None,
) -> Callable[[np.ndarray, np.ndarray, np.ndarray], np.ndarray]:
    ...
    def calc_flux(phi: np.ndarray, mu: np.ndarray, velocity_field: np.ndarray, out: np.ndarray) -> np.ndarray:
        ...
        for n in range(num_comp):  # iterate over all components
            ...
            for i in nb.prange(dim_x + 1):
                for j in range(1, dim_y + 1):
                    phi_staggered = (phi[n, i + 1, j] + phi[n, i, j]) / 2
                    velocity_staggered = (velocity_field[0, i+1, j] + velocity_field[0, i,j]) / 2
                    mu_grad_comp = -(mu[n, i + 1, j] - mu[n, i, j]) / dx
                    # print("v = ", velocity_field[0,i,j])
                    # print("v*phi = ",  velocity_field[0,i,j] * phi_staggered[n])
                    # print("ji = ", diff_arr[n] * phi_staggered * mu_grad_comp)
                    out[n, 0, i, j] = diff_arr[n] * phi_staggered * mu_grad_comp + velocity_staggered * phi_staggered  #  
                    
            # second space direction
            for i in nb.prange(1, dim_x + 1):
                for j in range(dim_y + 1):
                    phi_staggered = (phi[n, i, j + 1] + phi[n, i, j]) / 2
                    velocity_staggered = (velocity_field[1, i, j+1] + velocity_field[1, i,j]) / 2
                    mu_grad_comp = -(mu[n, i, j + 1] - mu[n, i, j]) / dy
                    out[n, 1, i, j] = diff_arr[n] * phi_staggered * mu_grad_comp +  velocity_staggered * phi_staggered # velocity_field[1,i,j] 
...
# def _make_mc_flux_to_staggered_advection_const_numba_2d(
#     grid: CartesianGrid,
#     num_comp: int,
...
#                     for i in range(2):  # iterate over all dimension
#                         # use diffusivities directly
#                         out[n, i, ...] += diff_arr[n, m] * mu_grads[m, i, ...] + velocity_field[i, ...] * phi[n]                  ############## ADD HYDRODYNAMIC FLOWS HERE I THINK (+V * PHI)
#         return out
...
def make_mc_flux_to_staggered_numba(
    ...
    if dim == 1:
        ...
        if mobility_model == "scaled_diagonal" and isinstance(velocity_field, np.ndarray):
            calc_flux = _make_mc_flux_to_staggered_advection_scaled_diagonal_numba_2d(
                grid, num_comp=num_comp, diffusivity=diffusivity
            )
        elif mobility_model == "scaled_correct":
